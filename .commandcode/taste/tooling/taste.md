...
- Never kill the live shell with `pkill -f` on a hand-typed binary name — the substring match can catch unrelated processes (editors/files open on the source, the pkill invocation itself). Kill by exact PID or exact process name instead; the C2 handover makes this an explicit do-not for live-gate restarts. Confidence: 0.6

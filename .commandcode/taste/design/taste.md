...
- When the plugin wire/host vocabulary cannot express a reference-UI element (no image node, no animation, no per-node color), render the honest approximation and record the delta as a deliberate, named deviation in the design ("honest visual deltas") — never fake it. Confidence: 0.65
- Extending the shared icon font appends new glyphs at fresh codepoints (the cellular set took 0xE03E–0xE046 after the device set's 0xE030–0xE03D) so every existing rune stays stable — never renumber; the rebuild's size delta gets recorded (9,076 → 10,444 bytes). Confidence: 0.7

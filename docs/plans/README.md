...
| `2026-09-26-system-monitor-panel-execution-handover.md` | execution-handover | Commissions the rest of the system monitor panel redesign on `feature/system-monitor-panel` (plan Tasks 1–11 done and reviewed, not merged or deployed): bd entries once the tracker is clean, seven deferred review minors and four open questions, one polish pass against the reference screenshot, plan Tasks 12–13 gated on sysc-clipboard and sysc-metrics releases, and the live Niri gate, merge and completion handover. Lists the rulings made during execution and the failures already present on `main`. |

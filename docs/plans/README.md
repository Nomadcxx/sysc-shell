...
| `2026-09-14-density-config-migration-design.md` | design | Keeps the 31 px current default and standard preset, restores the pre-rebase `standard` density as a hidden compatibility row, treats selector-free existing documents as legacy, and writes an explicit standard preset so new sparse files retain their generation across reload. |

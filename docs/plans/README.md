...
| `2026-09-19-launcher-mark.md` | plan | Executes D2 of the bar/weather/launcher/workspaces design: embed the supplied nested-gates PNG as a render asset and paint it as the bar launcher's image node. D1 and D3 remain unplanned. |
...
| `2026-09-25-bar-surface-execution-handover.md` | execution-handover | Commissions Phases B–E after Phase A (#15). Records the branch and PR state (the #13 branch tip needs resetting to `294c708`), the conversation-only decisions (frosted and attached defaults; Settings, launcher and clipboard float; segments and luminance lift deferred), and the facts not to rediscover: the `flags&1` capability bit, capability delivery before the first bar, `BlurShape` read in `renderJob`, the corrected concave fillet in `internal/ui/coverage.go`, the single `EdgeFillet`, the 4 px `filletMargin` cap, namespaces, and the four container-only test failures. Links the mockups and the reference-shell files read. |
...
| `2026-09-25-monitor-followups-execution-handover.md` | execution-handover | Commissions the follow-ups after the Control Centre Monitor page (sysc-517) and sysc-metrics `v0.6.0`, none of them treated as minor: pin `v0.6.0` and show VRAM first (sysc-522); GPU temperature in the Temperature row (sysc-526); a NaN guard (sysc-524) and allocation-free, benchmarked sparkline painting (sysc-523); `Window` on bar and standalone graphs (sysc-532); a GPU glyph (sysc-525); a GPU stale-line test (sysc-527); the root-device lookup off `Registry.mu` (sysc-528); and in sysc-metrics, integer memory parsing and tighter GPU tests for `v0.6.1` (sysc-529, sysc-530). Each item names its file anchor, fix and proof. |

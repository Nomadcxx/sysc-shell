...
Task 0 gates everything. Tasks 1–2 (icons) and Task 3 (configuration) are disjoint and may run in
parallel. Tasks 4–6 are serial.
...
**Interfaces:**
- Consumes: `Metrics`, `Source`, `Snapshot` from Tranche 3B.
...
func TestABatteryLeaseIsIndependentOfOtherSources(t *testing.T) {
	...
	lease, err := m.Acquire(SourceBattery, 30*time.Second)
	if err != nil {
		t.Fatalf("Acquire: %v", err)
	}
	defer lease.Release()

	if !m.Leased(SourceBattery) {
...
func TestOnlyTheBatterySourceIsPopulated(t *testing.T) {
	...
	lease, err := m.Acquire(SourceBattery, 50*time.Millisecond)
	if err != nil {
		t.Fatalf("Acquire: %v", err)
	}
	defer lease.Release()
...
and a value that moves a percent an hour would plot as a flat line.
...
**Interfaces:**
- Consumes: `metricSource` from Tranche 3B, `SourceBattery` from Task 2.
...
func TestABatteryWidgetLeasesTheBatterySource(t *testing.T) {
	...
	if !reg.Metrics().Leased(services.SourceBattery) {
...
Expected: FAIL — `metricSource` does not map `"battery"`, so no lease is taken.

- [ ] **Step 3: Write the implementation**

In `internal/shell/metricwidget.go`, add the case to `metricSource`:

```go
	case "battery":
		return services.SourceBattery, true
```

That is the whole change: `buildBar` already acquires a lease for every item `metricSource` recognises,

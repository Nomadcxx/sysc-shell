...
- The monitor page and search/filter controls are 28 px high. Column headers are 22 px high and read as table labels rather than filled pills. One rounded `FillContainerHigh` surface contains the virtual list. Each process uses a transparent 26 px row centred on a 32 px pitch; the virtual-list layout must preserve that explicit child height so the 6 px interval exists in arranged bounds. Typography and column alignment create the remaining rhythm instead of repeated chrome. The full row owns selection and focus; hover and press add a quiet flat state layer, and selection adds one rectangular `FillSoft` wash. The independently clickable outlined `Kill` action is 22 px high and no wider than its measured label needs.

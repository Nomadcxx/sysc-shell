...
| D9 | The process page provides All/User/System filters, search, sortable Name/CPU/Memory/PID columns, selectable highlighted rows, and a virtual list. One visible `Kill` action sends `SIGTERM`; the panel does not expose a disabled second-stage `SIGKILL` control. It reports permission and vanished-process failures inline. |
...
- Gauges are 22 px circular tracks with a rounded progress arc. CPU, memory, and GPU contain distinct project-owned monochrome vector glyphs sized for the ring; temperature contains one compact measured `NN°` run. Exact names and values appear in tooltips. Pointer action belongs to the shared capsule.
- The monitor page switch is one outlined segmented control. Process tools use a 32 px track; an explicit text-field height is honoured. Table headers are compact and centred. Process rows use a 40 px track with a visible gap, one quiet `Kill` action, and a persistent selected-row wash.

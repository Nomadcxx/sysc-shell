...
| D9 | The process page provides All/User/System filters, search, sortable Name/CPU/Memory/PID columns, selectable highlighted rows, and a virtual list. The full card owns selection and uses one edge-to-edge soft wash without an accent outline. One visible `Kill` action sends `SIGTERM`; the panel does not expose a disabled second-stage `SIGKILL` control. It reports permission and vanished-process failures inline. |
...
- The monitor page and search/filter controls are 28 px high. Column headers are 22 px high and read as table labels rather than filled pills. Process cards are 30 px high on a 38 px virtual-list pitch, leaving an 8 px gap. The full card owns the selection action and focus state; selected cards use one `FillSoft` wash without a separate rounded stroke. The independently clickable outlined `Kill` action is narrower and 24 px high.

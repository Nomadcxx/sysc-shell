...
func (s *Supervisor) Start(ctx context.Context) (*Session, error) {
	if p := s.Manifest.Protocol; p.Major != 1 || p.Minor > 7 {
		return nil, &IncompatibleError{Plugin: s.Manifest.ID, Want: 1, MaxMinor: 7, Got: p}
	}
...
func (s *Supervisor) handshake(ctx context.Context, sess *Session) error {
	...
	if reply.Protocol.Major != 1 || reply.Protocol.Minor > 7 {
		return &IncompatibleError{Plugin: s.Manifest.ID, Want: 1, MaxMinor: 7, Got: reply.Protocol}
	}
	want := hello.Plugin

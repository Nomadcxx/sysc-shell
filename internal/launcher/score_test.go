...
func TestScoreMatchesEveryApprovedField(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := rank([]Entry{tt.entry}, "needle")
			if len(got) != 1 {
...
func TestScoreAppliesFieldPenalty(t *testing.T) {
	...
	name := rank([]Entry{{Name: "Needle"}}, "needle")
	comment := rank([]Entry{{Name: "Other", Comment: "Needle"}}, "needle")
...
func TestScoreDoesNotSearchDesktopActions(t *testing.T) {
	...
	got := rank([]Entry{{Name: "Editor", Actions: []Action{{Name: "Needle"}}}}, "needle")
	if len(got) != 0 {

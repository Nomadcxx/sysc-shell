...
func TestDefaultVocabularyShipsTimeDateGroupWordmarkAndMedia(t *testing.T) {
	...
	for _, item := range cfg.Bar.Right {
		...
		for _, m := range members {
			if m.ID == "notifications" || m.ID == "running-apps" || m.ID == "clipboard" {
				continue
			}
			if m.Interval <= 0 {
...
func TestBuiltInBarOmitsBatteryItem(t *testing.T) {
	t.Parallel()
	for _, item := range Default().Bar.Right {
		if item.ID != "battery" {
			continue
		}
		t.Fatal("built-in desktop bar includes a battery item")
	}
}
...
func TestThemePresetSeedsEveryAxis(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"standard", theme.DensityStandard, 12, theme.MotionStandard, 100, 100, true},

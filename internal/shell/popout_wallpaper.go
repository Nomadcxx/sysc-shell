...
func wallpaperTree(r *Registry, h *PanelHost) *ui.Node {
	...
	list := &ui.Node{
		...
		Item: func(row int) *ui.Node {
			return wallpaperRow(h, entries, row)
		},
	}
...
func wallpaperRow(h *PanelHost, entries []wallpaper.Entry, row int) *ui.Node {
	start := row * wallpaperColumns
	tiles := make([]*ui.Node, 0, wallpaperColumns)
	for i := start; i < start+wallpaperColumns && i < len(entries); i++ {
		tiles = append(tiles, wallpaperTile(h, entries[i], i))
	}
...
func wallpaperTile(h *PanelHost, entry wallpaper.Entry, index int) *ui.Node {
	thumb := &ui.Node{
		Kind:   ui.KindImage,

...
// chipCount is how many chips a configured bar owes: one per widget, plus one
// handle for each group. It is counted from the configuration rather than
// written down, so a bar that gains or loses a default widget does not make
// this a test about arithmetic.
func chipCount(bar config.Bar) int {
	var total int
	for _, lane := range [][]config.Item{bar.Left, bar.Center, bar.Right} {
...
func TestTheDefaultBarFitsTheStrip(t *testing.T) {
	...
	h := barHost(t, bar)
	got := chipNames(barLaneStrip(h))
	if want := chipCount(bar); len(got) != want {
		t.Errorf("default bar drew %d chips (%v), want %d: one per widget plus each group's handle", len(got), got, want)
	}
}
